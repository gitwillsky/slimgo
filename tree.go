	...
	walk:
		for {
			...
			if i < len(path) {
				...
				n.insertChild(numParams, path, regPath, handlers)
				return
	...
	for i, max := 0, len(path); numParams > 0; i++ {
		...
		if len(n.children) > 0 {
			panic("wildcard route '" + path[i:end] +
				"' conflicts with existing children in path '" + regPath + "'")
		}

		// check if the wildcard has a name
		if end-i < 2 {
			panic("wildcards must be named with a non-empty name in path '" + regPath + "'")
		}
		...
		if c == ':' { // param
			...
			if end != max || numParams > 1 {
				panic("catch-all routes are only allowed at the end of the path in path '" + regPath + "'")
			}

			if len(n.path) > 0 && n.path[len(n.path)-1] == '/' {
				panic("catch-all conflicts with existing handle for the path segment root in path '" + regPath + "'")
			}

			// currently fixed width 1 for '/'
			i--
			if path[i] != '/' {
				panic("no / before catch-all in path '" + regPath + "'")
			}
...
walk: // outer loop for walking the tree
	for {
		if len(path) > len(n.path) {
			...
			if path == "/" && n.wildChild && n.nType != rootType {
				tsr = true
				return
			}
			...
			for i := 0; i < len(n.indices); i++ {
				if n.indices[i] == '/' {
					...
					tsr = (len(n.path) == 1 && n.value != nil) ||
						(n.nType == catchAllType && n.children[0].value != nil)
					return
				}
			}
	...
	for len(loPath) >= len(loNPath) && (len(loNPath) == 0 || loPath[1:len(loNPath)] == loNPath[1:]) {
		...
		if path = path[len(n.path):]; len(path) > 0 {
			...
			if !n.wildChild {
				...
				return ciPath, (fixTrailingSlash && path == "/" && n.value != nil)
			}
